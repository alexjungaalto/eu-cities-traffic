"""Pixel-wise segmentation of central Bologna using OpenStreetMap.
...
free open-WFS surface dataset exists for Bologna. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Piazza Maggiore. Captures the porticoed historic
# centre, the railway station and a margin of the modern fabric.
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-bologna-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("bologna_osm_labels.png")
Image.fromarray(overlay).save("bologna_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Bologna OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved bologna_osm_labels.png  bologna_osm_overlay.png")
