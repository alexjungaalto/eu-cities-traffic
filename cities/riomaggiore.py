"""Pixel-wise segmentation of central Riomaggiore using OpenStreetMap.
...
free open-WFS surface dataset exists for Riomaggiore. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 1.5 x 1.5 km centred on Riomaggiore. Captures the village, the harbour and the southern stretch of the coastal trail.
LAT0, LON0 = 44.0997, 9.7372
SIZE_M = 1500
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-riomaggiore-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("riomaggiore_osm_labels.png")
Image.fromarray(overlay).save("riomaggiore_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Riomaggiore OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved riomaggiore_osm_labels.png  riomaggiore_osm_overlay.png")
