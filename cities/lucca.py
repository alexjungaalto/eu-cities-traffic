"""Pixel-wise segmentation of central Lucca using OpenStreetMap.
...
free open-WFS surface dataset exists for Lucca. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 3 x 3 km centred near Piazza San Michele. Lucca's walled centro storico
# is ~1.2 km across; the wider AOI captures the walls and the surrounding
# 19th/20th-century fabric.
LAT0, LON0 = 43.8430, 10.5050
SIZE_M = 3000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-lucca-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("lucca_osm_labels.png")
Image.fromarray(overlay).save("lucca_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Lucca OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved lucca_osm_labels.png  lucca_osm_overlay.png")
