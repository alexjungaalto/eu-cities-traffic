"""Pixel-wise segmentation of central Prague using OpenStreetMap.
...
free open-WFS surface dataset exists for Prague. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Staroměstské náměstí. Covers the historic core on both Vltava banks, Malá Strana, Wenceslas Square and Vyšehrad.
LAT0, LON0 = 50.0875, 14.4214
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-prague-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("prague_osm_labels.png")
Image.fromarray(overlay).save("prague_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Prague OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved prague_osm_labels.png  prague_osm_overlay.png")
