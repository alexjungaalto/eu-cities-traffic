"""Pixel-wise segmentation of central Mariazell using OpenStreetMap.
...
free open-WFS surface dataset exists for Mariazell. The trade-off:
...
reprojected to UTM 33N (EPSG:32633) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 3 x 3 km centred on the Basilica. Covers the pilgrimage centre, Erlaufsee road and the Mariazellerbahn station.
LAT0, LON0 = 47.7717, 15.317
SIZE_M = 3000
...
# Reproject AOI corners to UTM 33N for the metric grid.
_aoi = gpd.GeoSeries.from_xy(
    [lon_min, lon_max, lon_min, lon_max],
    [lat_min, lat_min, lat_max, lat_max], crs="EPSG:4326").to_crs(32633)
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-mariazell-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32633)
...
Image.fromarray(seg).save("mariazell_osm_labels.png")
Image.fromarray(overlay).save("mariazell_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Mariazell OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved mariazell_osm_labels.png  mariazell_osm_overlay.png")
