"""Pixel-wise segmentation of central Brussels using OpenStreetMap.
...
free open-WFS surface dataset exists for Brussels. The trade-off:
...
reprojected to UTM 31N (EPSG:32631) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred near the Grand-Place / Grote Markt. Covers the
# Pentagon (medieval centre inside the old walls), the European
# Quarter and a fringe of Saint-Gilles / Ixelles.
...
_aoi = gpd.GeoSeries.from_xy(
    ...
    [lat_min, lat_min, lat_max, lat_max], crs="EPSG:4326").to_crs(32631)
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-brussels-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32631)
...
Image.fromarray(seg).save("brussels_osm_labels.png")
Image.fromarray(overlay).save("brussels_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Brussels OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved brussels_osm_labels.png  brussels_osm_overlay.png")
