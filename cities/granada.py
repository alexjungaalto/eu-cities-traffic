"""Pixel-wise segmentation of central Granada using OpenStreetMap.
...
free open-WFS surface dataset exists for Granada. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred near Plaza Nueva. Captures the Albayzín, the Realejo and the foot of the Alhambra.
LAT0, LON0 = 37.1773, -3.5986
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-granada-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("granada_osm_labels.png")
Image.fromarray(overlay).save("granada_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Granada OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved granada_osm_labels.png  granada_osm_overlay.png")
