"""Pixel-wise segmentation of central Moneglia using OpenStreetMap.
...
free open-WFS surface dataset exists for Moneglia. The trade-off:
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-moneglia-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("moneglia_osm_labels.png")
Image.fromarray(overlay).save("moneglia_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Moneglia OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved moneglia_osm_labels.png  moneglia_osm_overlay.png")
