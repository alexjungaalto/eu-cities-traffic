"""Pixel-wise segmentation of central Bratislava using OpenStreetMap.
...
free open-WFS surface dataset exists for Bratislava. The trade-off:
...
reprojected to UTM 34N (EPSG:32633) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Hviezdoslavovo námestie. Covers Staré Mesto, the Danube banks, the castle hill and a slice of the modern centre.
LAT0, LON0 = 48.1428, 17.11
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-bratislava-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("bratislava_osm_labels.png")
Image.fromarray(overlay).save("bratislava_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Bratislava OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved bratislava_osm_labels.png  bratislava_osm_overlay.png")
