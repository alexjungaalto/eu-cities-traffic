"""Pixel-wise segmentation of central Graz using OpenStreetMap.
...
free open-WFS surface dataset exists for Graz. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Hauptplatz. Captures the UNESCO Altstadt with the
# Schlossberg, the Mur riverfront, the main station district (Lend) and
# parts of Geidorf / Jakomini.
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-graz-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("graz_osm_labels.png")
Image.fromarray(overlay).save("graz_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Graz OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved graz_osm_labels.png  graz_osm_overlay.png")
