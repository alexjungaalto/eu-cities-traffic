"""Pixel-wise segmentation of central Turku using OpenStreetMap.
...
free open-WFS surface dataset exists for Turku. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Kauppatori. Captures the Aurajoki riverfront, the cathedral and a slice of the harbour.
LAT0, LON0 = 60.4518, 22.2666
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-turku-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("turku_osm_labels.png")
Image.fromarray(overlay).save("turku_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Turku OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved turku_osm_labels.png  turku_osm_overlay.png")
