"""Pixel-wise segmentation of central Pristina using OpenStreetMap.
...
free open-WFS surface dataset exists for Pristina. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Skanderbeg Square. Covers the Newborn monument area, Boulevard Bill Clinton and the Velania/Dragodan neighbourhoods.
LAT0, LON0 = 42.6629, 21.1655
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-pristina-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("pristina_osm_labels.png")
Image.fromarray(overlay).save("pristina_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Pristina OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved pristina_osm_labels.png  pristina_osm_overlay.png")
