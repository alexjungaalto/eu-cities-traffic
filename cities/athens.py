"""Pixel-wise segmentation of central Athens using OpenStreetMap.
...
free open-WFS surface dataset exists for Athens. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Syntagma Square. Captures the Acropolis, Plaka,
# Monastiraki, Omonia and most of the historic Athenian core.
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-athens-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("athens_osm_labels.png")
Image.fromarray(overlay).save("athens_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Athens OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved athens_osm_labels.png  athens_osm_overlay.png")
