"""Pixel-wise segmentation of central Monterosso using OpenStreetMap.
...
free open-WFS surface dataset exists for Monterosso. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 1.5 x 1.5 km centred on Monterosso al Mare. Captures the village and the beachfront.
LAT0, LON0 = 44.1463, 9.6541
SIZE_M = 1500
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-monterosso-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("monterosso_osm_labels.png")
Image.fromarray(overlay).save("monterosso_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Monterosso OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved monterosso_osm_labels.png  monterosso_osm_overlay.png")
