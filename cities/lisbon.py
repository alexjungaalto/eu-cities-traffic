"""Pixel-wise segmentation of central Lisbon using OpenStreetMap.
...
free open-WFS surface dataset exists for Lisbon. The trade-off:
...
reprojected to UTM 29N (EPSG:32629) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred near Praça do Comércio. Captures Baixa, Alfama,
# Bairro Alto, Chiado and a strip of the Tejo waterfront.
LAT0, LON0 = 38.7100, -9.1380
SIZE_M = 4000
...
_aoi = gpd.GeoSeries.from_xy(
    ...
    [lat_min, lat_min, lat_max, lat_max], crs="EPSG:4326").to_crs(32629)
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-lisbon-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32629)
...
Image.fromarray(seg).save("lisbon_osm_labels.png")
Image.fromarray(overlay).save("lisbon_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Lisbon OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved lisbon_osm_labels.png  lisbon_osm_overlay.png")
