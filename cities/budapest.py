"""Pixel-wise segmentation of central Budapest using OpenStreetMap.
...
free open-WFS surface dataset exists for Budapest. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Deák Ferenc tér. Spans both Buda and Pest across the Danube, Várnegyed castle hill, Belváros and the Andrássy axis up to Oktogon.
LAT0, LON0 = 47.4979, 19.054
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-budapest-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("budapest_osm_labels.png")
Image.fromarray(overlay).save("budapest_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Budapest OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved budapest_osm_labels.png  budapest_osm_overlay.png")
