"""Pixel-wise segmentation of central Siena using OpenStreetMap.
...
free open-WFS surface dataset exists for Siena. The trade-off:
...
reprojected to UTM 32N (EPSG:32632) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 3 x 3 km centred on Piazza del Campo. Siena's centro storico is ~1 x 1 km;
# the wider AOI captures the medieval walls and the surrounding modern fabric.
LAT0, LON0 = 43.3186, 11.3306
SIZE_M = 3000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-siena-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("siena_osm_labels.png")
Image.fromarray(overlay).save("siena_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Siena OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved siena_osm_labels.png  siena_osm_overlay.png")
