"""Pixel-wise segmentation of central Tallinn using OpenStreetMap.
...
free open-WFS surface dataset exists for Tallinn. The trade-off:
...
reprojected to UTM 35N (EPSG:32635) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Raekoja plats. Covers the medieval Old Town inside the walls, the Rotermann quarter and the seaside passenger port.
LAT0, LON0 = 59.437, 24.7536
SIZE_M = 4000
...
# Reproject AOI corners to UTM 35N for the metric grid.
_aoi = gpd.GeoSeries.from_xy(
    [lon_min, lon_max, lon_min, lon_max],
    [lat_min, lat_min, lat_max, lat_max], crs="EPSG:4326").to_crs(32635)
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-tallinn-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32635)
...
osm = _geocache.cached_json(NAME, "osm_main", lambda: overpass(f"""
...
Image.fromarray(seg).save("tallinn_osm_labels.png")
Image.fromarray(overlay).save("tallinn_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Tallinn OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved tallinn_osm_labels.png  tallinn_osm_overlay.png")
