"""Pixel-wise segmentation of central Vernazza using OpenStreetMap.
...
free open-WFS surface dataset exists for Vernazza. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 1.5 x 1.5 km centred on Vernazza. Captures the harbour, the medieval core and the terraced hillsides.
LAT0, LON0 = 44.1352, 9.684
SIZE_M = 1500
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-vernazza-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("vernazza_osm_labels.png")
Image.fromarray(overlay).save("vernazza_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Vernazza OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved vernazza_osm_labels.png  vernazza_osm_overlay.png")
