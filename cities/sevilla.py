"""Pixel-wise segmentation of central Sevilla using OpenStreetMap.
...
free open-WFS surface dataset exists for Sevilla. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred near the cathedral. Covers the Casco Antiguo, Triana across the Guadalquivir, and Plaza de España.
LAT0, LON0 = 37.3886, -5.9823
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-sevilla-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("sevilla_osm_labels.png")
Image.fromarray(overlay).save("sevilla_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Sevilla OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved sevilla_osm_labels.png  sevilla_osm_overlay.png")
