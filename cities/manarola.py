"""Pixel-wise segmentation of central Manarola using OpenStreetMap.
...
free open-WFS surface dataset exists for Manarola. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 1.5 x 1.5 km centred on Manarola, with its iconic harbour cliff and the Via dell'Amore trailhead.
LAT0, LON0 = 44.1066, 9.7273
SIZE_M = 1500
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-manarola-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("manarola_osm_labels.png")
Image.fromarray(overlay).save("manarola_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Manarola OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved manarola_osm_labels.png  manarola_osm_overlay.png")
