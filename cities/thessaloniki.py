"""Pixel-wise segmentation of central Thessaloniki using OpenStreetMap.
...
free open-WFS surface dataset exists for Thessaloniki. The trade-off:
...
reprojected to UTM 34N (EPSG:32634) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Aristotelous Square. Captures the seafront, the
# Roman Forum, Ano Poli (upper town) and the White Tower waterfront.
LAT0, LON0 = 40.6332, 22.9410
SIZE_M = 4000
...
_aoi = gpd.GeoSeries.from_xy(
    ...
    [lat_min, lat_min, lat_max, lat_max], crs="EPSG:4326").to_crs(32634)
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-thessaloniki-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32634)
...
Image.fromarray(seg).save("thessaloniki_osm_labels.png")
Image.fromarray(overlay).save("thessaloniki_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Thessaloniki OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved thessaloniki_osm_labels.png  thessaloniki_osm_overlay.png")
