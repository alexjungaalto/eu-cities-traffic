"""Pixel-wise segmentation of central Málaga using OpenStreetMap.
...
free open-WFS surface dataset exists for Málaga. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred near Plaza de la Constitución. Captures the casco
# histórico, the Alcazaba/Gibralfaro hill, the port and a strip of the
# Ensanche grid.
LAT0, LON0 = 36.7213, -4.4214
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-malaga-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("malaga_osm_labels.png")
Image.fromarray(overlay).save("malaga_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Málaga OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved malaga_osm_labels.png  malaga_osm_overlay.png")
