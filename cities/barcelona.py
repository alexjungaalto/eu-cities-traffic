"""Pixel-wise segmentation of central Barcelona using OpenStreetMap.
...
free open-WFS surface dataset exists for Barcelona. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Plaça de Catalunya. Captures Ciutat Vella (Gothic
# Quarter, Born, Raval) and a large slice of the Eixample grid.
LAT0, LON0 = 41.3870, 2.1700
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-barcelona-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("barcelona_osm_labels.png")
Image.fromarray(overlay).save("barcelona_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Barcelona OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved barcelona_osm_labels.png  barcelona_osm_overlay.png")
