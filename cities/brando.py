"""Pixel-wise segmentation of central Brändö using OpenStreetMap.
...
free open-WFS surface dataset exists for Brändö. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on the Brändö (Åland) main village. Captures the church, harbour and a slice of the surrounding archipelago.
LAT0, LON0 = 60.45, 21.05
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-brando-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("brando_osm_labels.png")
Image.fromarray(overlay).save("brando_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Brändö OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved brando_osm_labels.png  brando_osm_overlay.png")
