"""Pixel-wise segmentation of central Belgrade using OpenStreetMap.
...
free open-WFS surface dataset exists for Belgrade. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Trg Republike. Covers Stari Grad, Knez Mihailova pedestrian street, Kalemegdan fortress and a slice of Savamala.
LAT0, LON0 = 44.8154, 20.4609
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-belgrade-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("belgrade_osm_labels.png")
Image.fromarray(overlay).save("belgrade_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Belgrade OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved belgrade_osm_labels.png  belgrade_osm_overlay.png")
