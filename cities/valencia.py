"""Pixel-wise segmentation of central Valencia using OpenStreetMap.
...
free open-WFS surface dataset exists for Valencia. The trade-off:
...
reprojected to UTM 30N (EPSG:32630) for the metric raster grid.
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Plaza del Ayuntamiento. Covers Ciutat Vella, the Mercat Central area and a slice of the Eixample.
...
# Reproject AOI corners to UTM 30N for the metric grid.
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-valencia-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
def osm_to_gdf(osm, want_geom_types):
    ...
    return gpd.GeoDataFrame(rows, crs="EPSG:4326").to_crs(32630)
...
Image.fromarray(seg).save("valencia_osm_labels.png")
Image.fromarray(overlay).save("valencia_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Valencia OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved valencia_osm_labels.png  valencia_osm_overlay.png")
