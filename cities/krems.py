"""Pixel-wise segmentation of central Krems using OpenStreetMap.
...
free open-WFS surface dataset exists for Krems. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 3 x 3 km centred near the Krems town hall / Hoher Markt. Captures the
# Altstadt, the Steiner Tor area, IMC Krems campus and a slice of the
# Donau riverfront.
LAT0, LON0 = 48.4108, 15.6080
SIZE_M = 3000
...
# Reproject AOI corners to UTM 32N for the metric grid.
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-krems-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("krems_osm_labels.png")
Image.fromarray(overlay).save("krems_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Krems OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved krems_osm_labels.png  krems_osm_overlay.png")
