"""Pixel-wise segmentation of central Sofia using OpenStreetMap.
...
free open-WFS surface dataset exists for Sofia. The trade-off:
...
# --- Area Of Interest -------------------------------------------------------
# 4 x 4 km centred on Sveta Nedelya square. Covers the centre with the Alexander Nevsky cathedral, Vitosha boulevard and the National Palace of Culture.
LAT0, LON0 = 42.6977, 23.3219
SIZE_M = 4000
    ...
    for url in OVERPASS_MIRRORS:
        try:
            r = requests.post(url, data={"data": query},
                              headers={"User-Agent": "kbw-sofia-demo/1.0"}, timeout=300)
            r.raise_for_status()
            return r.json()
...
Image.fromarray(seg).save("sofia_osm_labels.png")
Image.fromarray(overlay).save("sofia_osm_overlay.png")

px = lambda m: int(m.sum() * PX**2)
aoi = (maxx - minx) * (maxy - miny)
print(f"\n=== Sofia OSM segmentation ({(maxx-minx):.0f}m x {(maxy-miny):.0f}m AOI) ===")
...
print("Saved sofia_osm_labels.png  sofia_osm_overlay.png")
